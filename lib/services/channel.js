...
const { getGeoIPInfo } = require('./geoip.js');
...
async function funcCreateSession({ username }, channel, arguments) {
...
async function commandLookup(user, channel, arguments) {
  ...
  if (player.current_match.id > 0) {
    ...
    ingame = `${(map ? map.name : "picking-phase")} - ${player.current_match.team1_score}-${player.current_match.team2_score} - https://esportal.com/sv/match/${player.current_match.id}`
  }
...
async function commandGeoIP(user, channel, arguments) {
  const json = await getGeoIPInfo(arguments);
  if(json.status === "fail") {
    channel.send(`${arguments}: failed to check IP (${json.message})`);
    return;
  }
  channel.send(`${arguments}: ${json.city}, ${json.regionName}, ${json.country} (${json.isp})`)
}
