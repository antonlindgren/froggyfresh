...
async function commandLookup(user, channel, arguments) {
  ...
  if (player.current_match.id > 0) {
    ingame = `${player.current_match.team1_score}-${player.current_match.team2_score}`
  }

  console.log(player.current_match)

  channel.send(
    `
    >>> **${arguments}**: :thumbsup: ${player.thumbs_up} :thumbsdown: ${
      player.thumbs_down
    } **Elo**: ${player.elo} **Games**: ${
      player.wins + player.losses
    } (**Win ratio**: ${Math.round(
      (player.wins / (player.wins + player.losses)) * 100
    )})

  ${ingame !== '' ? ':red_circle:' + ingame : ''}
    `
  )
}
