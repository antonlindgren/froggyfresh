...
async function esportalCron() {
  ...
  for await (const player of players) {
    const playerData = await getUserInfo(player.esportal_id, ['current_match']);
    ...
    if(playerData.current_match.id > lastAnnounceId) {
        ...
        if(!exist) {
			matches.push({
				...
				players: [],
				map: 'picking-phase',
				map_image: 'https://static2.esportal.com/5swjas38sMbMtg/full.png',
				score: '',
				mvp_user_id: 0,
				is_current: true
			})
  ...
  for await (const match of matches) {
	...
	match.mvp_user_id = matchData.mvp_user_id;
    const map = maps.find(({ id: mId }) => mId === matchData.map_id)
	if(map) {
		match.map = map.name
		match.map_image = 'https://static2.esportal.com/' + map.image.replace('.png', '/full.png')
	}
      ...
      match.players.forEach((p, idx) => {
        if (p.nickname.toLowerCase() === player.username.toLowerCase()) {
          match.players[idx].stats =
            ...
            player.assists +
            'A - ' +
            (player.kills / player.deaths).toFixed(2) + ' KD (' +
            player.clutches +
            ' clutches, ' +
      ...
      embeds: [
        {
          ...
          description:
            players.join(', ') +
            ' has '+(match.is_current === true ? "started" : "finished")+ ' a match '+(match.is_current === true ? ":open_mouth:" : "😍")+'.\nThe match '+(match.is_current === true ? "score is" : "ended with score")+ ' '+
            match.score +
            ' on map ' +
            match.map,
          fields: playerFields
